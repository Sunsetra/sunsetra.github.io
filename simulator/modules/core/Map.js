...
class Map {
  constructor(data, resList) {
    ...
    const uvs = []; // 存放顶点UV信息
    const sideGroup = []; // 侧面贴图顶点组信息，每个元组是一个组的[start, count]
    const faces = [
      {
        normal: [-1, 0, 0],
        corners: [
    ...
    mapGeometry.setAttribute('position', new THREE.BufferAttribute(new Float32Array(positions), 3));
    mapGeometry.setAttribute('normal', new THREE.BufferAttribute(new Float32Array(normals), 3));
    mapGeometry.setAttribute('uv', new THREE.BufferAttribute(new Float32Array(uvs), 2));
    mapGeometry.setIndex(indices);
    const materialList = []; // 创建所需的所有砖块表面贴图材质
    const materialMap = {}; // 材质类型在材质列表中的索引
    data.resources.block.forEach((type) => {
      const res = resList.block[type];
      if (res.mat && res.mat instanceof THREE.Material) {
    ...
    this.mesh = new THREE.Mesh(mapGeometry, materialList);
